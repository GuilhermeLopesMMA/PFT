...
if (!atletaId) {
    document.getElementById('atletaContent').innerHTML = `
        <div style="text-align: center; padding: 3rem;">
            ...
            <a href="Atletas.html" style="color: #1fa036; text-decoration: none; margin-top: 1rem; display: inline-block;">← Voltar para Atletas</a>
        </div>
    `;
} else {
...
function renderizarPerfil(atleta) {
    ...
    document.title = `${atleta.nome} - PFT`;
    ...
    let fotoHTML;
    if (atleta.foto_url) {
        fotoHTML = `<img src="${atleta.foto_url}" alt="${atleta.nome}" style="width: 300px; height: 300px; object-fit: cover; border-radius: 12px; border: 3px solid #1fa036; box-shadow: 0 8px 16px rgba(31, 160, 54, 0.3);">`;
    } else {
        const inicial = atleta.nome.charAt(0).toUpperCase();
        fotoHTML = `<div style="width: 300px; height: 300px; background: linear-gradient(135deg, #1fa036 0%, #0c6e22 100%); border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 6rem; color: white; font-weight: 700; border: 3px solid #1fa036; box-shadow: 0 8px 16px rgba(31, 160, 54, 0.3);">
            ${inicial}
        </div>`;
    }
    ...
    const html = `
        <h1>${atleta.nome}</h1>
        <p style="text-align: center; font-size: 1.2rem; color: #1fa036; margin-bottom: 2rem;">
        ...
        <div style="text-align: center; margin-top: 2rem;">
            <a href="Atletas.html" style="color: #1fa036; text-decoration: none; font-size: 1.1rem; padding: 0.75rem 2rem; border: 2px solid #1fa036; border-radius: 8px; display: inline-block; transition: all 0.3s;">
                ← Voltar para Atletas
            </a>
        </div>
    `;
