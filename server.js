...
// ============================================
// GUARDAS DE PROCESSO
// Sem isto, QUALQUER erro async não tratado mata o servidor.
// Esta era uma das causas dos crashes constantes.
// ============================================
process.on("unhandledRejection", (motivo) => {
  console.error("[unhandledRejection] Promise rejeitada sem catch:", motivo);
  // Não encerramos o processo: o servidor continua a servir os outros pedidos.
});

process.on("uncaughtException", (erro) => {
  console.error("[uncaughtException] Erro não apanhado:", erro);
  // Também não encerramos: evita o loop de restarts no Render.
});
...
if (IS_PROD && JWT_SECRET.length < 32) {
  console.error("ERRO FATAL: JWT_SECRET demasiado curto para produção (mínimo 32 caracteres).");
  process.exit(1);
}

const app = express();

// Necessário no Render (está atrás de um proxy) para obter o IP e o protocolo reais
...
// Antes: express.static(__dirname) — expunha .env, server.js, models/, package.json.
// Qualquer pessoa podia descarregar as tuas credenciais.
app.use(
...
// ============================================
// UPLOAD DE FOTOS (MULTER) — ARMAZENAMENTO EM MEMÓRIA
//
// O disco do Render é EFÉMERO: tudo o que escreves em ./media/atletas
// desaparece a cada restart ou novo deploy. Por isso as fotos passam a
// ser guardadas na base de dados (MongoDB) e servidas por /api/foto/:id.
// ============================================
...
const upload = multer({
  ...
  limits: { fileSize: 3 * 1024 * 1024 } // 3MB máximo (documentos MongoDB: limite 16MB)
});
...
app.post("/api/verificar-email", async (req, res) => {
  try {
    ...
    res.json({
      mensagem: "Email verificado com sucesso! Bem-vindo à PFT!",
      token,
      utilizador: {
        id: utilizador._id,
...
// A imagem é guardada na base de dados (o disco do Render é efémero).
app.post("/api/perfil/foto", autenticarToken, upload.single('foto'), async (req, res) => {
  try {
    ...
    utilizador.foto_data = req.file.buffer;
    utilizador.foto_mime = req.file.mimetype;
    // Cache-busting: muda o URL sempre que a foto muda, para o browser recarregar.
    utilizador.foto_url = `/api/foto/${userId}?v=${Date.now()}`;

    await utilizador.save();

    console.log(`Foto atualizada: ${utilizador.nome} (${req.file.size} bytes)`);
    ...
    res.status(500).json({ erro: 'Erro ao fazer upload de foto' });
  }
});

// ========== ROTA PÚBLICA - SERVIR FOTO A PARTIR DA BD ==========
app.get("/api/foto/:id", async (req, res) => {
  try {
    const utilizador = await User.findById(req.params.id).select('+foto_data +foto_mime');

    if (!utilizador || !utilizador.foto_data) {
      return res.status(404).json({ erro: 'Foto não encontrada' });
    }

    res.set('Content-Type', utilizador.foto_mime || 'image/jpeg');
    res.set('Cache-Control', 'public, max-age=86400');
    return res.send(utilizador.foto_data);

  } catch (error) {
    // ID inválido (CastError) ou outro erro — devolve 404 em vez de crashar
    return res.status(404).json({ erro: 'Foto não encontrada' });
  }
});
...
app.delete("/api/perfil/foto", autenticarToken, async (req, res) => {
  try {
    ...
    utilizador.foto_data = null;
    utilizador.foto_mime = null;
    utilizador.foto_url = null;
    await utilizador.save();

    console.log('Foto removida:', utilizador.nome);
...
// ============================================
// HEALTH CHECK (usado pelo Render para saber se a app está viva)
// ============================================
app.get("/healthz", (req, res) => {
  const mongoose = require("mongoose");
  const estados = ["desligado", "ligado", "a ligar", "a desligar"];
  const dbEstado = estados[mongoose.connection.readyState] || "desconhecido";

  const saudavel = mongoose.connection.readyState === 1;
...
// ============================================
// 404
// A API devolve JSON. As páginas devolvem o index.html.
// ============================================
app.use((req, res) => {
...
//
// IMPORTANTE: tem de vir DEPOIS de todas as rotas.
// No código antigo estava declarado no topo, por isso NUNCA apanhava nada
// — e qualquer erro numa rota rebentava o processo.
// ============================================
app.use((error, req, res, next) => {
  if (error instanceof multer.MulterError) {
    if (error.code === "LIMIT_FILE_SIZE") {
      return res.status(400).json({ erro: "Ficheiro muito grande! Máximo: 3MB" });
    }
  ...
  res.status(error.status || 500).json({
    erro: IS_PROD ? "Erro interno do servidor" : error.message
  });
});

// ============================================
// ARRANQUE
// A base de dados liga PRIMEIRO; só depois abrimos a porta.
// Assim o Render nunca encaminha tráfego para uma app sem BD.
// ============================================
...
// ============================================
// ENCERRAMENTO LIMPO
// O Render envia SIGTERM antes de reiniciar/parar o serviço.
// ============================================
